    ...
    return (
      <div className={`text-${props.mode==='light'?'dark':'light'}`}>
        ...
        cursor:''}}
          ...
          <button className="btn btn-primary mx-1" onClick={onCapitalizeclick}>
            Capitalize a first later
          </button>
          <button className="btn btn-primary mx-1" onClick={onHandleCopy}>
            Copy Text
          </button>
          <button className="btn btn-primary mx-1" onClick={onRemoveExtra}>
          Remove Extra 
          </button>
        
        <h2>Summary:</h2>
      
        <p><b>{text.split(" ").length-1}</b> words</p>
        <p><b>{text.split('').length}</b> character</p>
        <p><b>{0.0008*text.length} </b>Minutes to read</p>
        <h2>Preview</h2>
